...
@app.post("/api/path")
def api_path():
    """Stream a learning path for any topic (basics -> advanced)."""
    data = request.json or {}
...
def _build_path_pdf(topic: str, md: str) -> bytes:
    ...
    def line(text, size, style="", gap_before=0):
        if gap_before:
            pdf.ln(gap_before)
        pdf.set_font("Helvetica", style, size)
        pdf.multi_cell(0, size * 0.5 + 1, _pdf_sanitize(text),
                       new_x="LMARGIN", new_y="NEXT")

    line(f"Learning Path: {topic}", 18, "B")
    pdf.ln(2)
    for raw in md.replace("\r", "").split("\n"):
        ...
        if not t:
            ...
            line(_strip_md(t[4:]), 12, "B", gap_before=1)
        elif t.startswith("## "):
            line(_strip_md(t[3:]), 14, "B", gap_before=2)
        elif t.startswith("# "):
            line(_strip_md(t[2:]), 16, "B", gap_before=2)
        elif t[:2] in ("- ", "* "):
            line("   -  " + _strip_md(t[2:]), 11)
        else:
            line(_strip_md(t), 11)
    pdf.ln(5)
    line("Generated by Study Tutor", 9, "I")
    return bytes(pdf.output())


@app.post("/api/path/pdf")
def api_path_pdf():
    data = request.json or {}
    topic = (data.get("topic", "") or "Learning Path").strip()
    md = data.get("md", "") or data.get("markdown", "")
    if not md:
        return jsonify({"error": "no content"}), 400
    try:
        pdf_bytes = _build_path_pdf(topic, md)
    except Exception as exc:
        return jsonify({"error": f"pdf failed: {exc}"}), 500
    safe = re.sub(r"[^A-Za-z0-9]+", "_", topic).strip("_")[:40] or "learning_path"
    return Response(
